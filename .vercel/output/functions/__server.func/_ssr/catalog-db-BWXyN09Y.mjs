...
//#region node_modules/.nitro/vite/services/ssr/assets/catalog-db-BWXyN09Y.js
...
async function ensureCommerceSchema(sql) {
	await sql.query(`
...
async function seedCatalog(sql) {
	...
	for (const p of PRODUCTS) await sql`
...
function parseProduct(payload, slug) {
	try {
...
async function loadCatalog(sql, publishedOnly = true) {
	await seedCatalog(sql);
	const rows = publishedOnly ? await sql`
...
//#endregion
export { seedCatalog as i, createServerRpc as n, loadCatalog as r, catalogMap as t };
